...
searchBtn.addEventListener('click', function () {
    ...
    fetch(forecastFiveInput)
        ...
        .then(function (data) {
            console.log(data)
            var fullDateList = data.list;
            var dateList = [];
            var maxVal = 5;
            var everyEighth = Math.floor(fullDateList.length / maxVal);
            for (i = 0; i < fullDateList.length; i = i + everyEighth) {
                dateList.push(fullDateList[i]);
